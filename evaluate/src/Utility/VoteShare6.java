...
public class VoteShare6 {
    public static void main(String[] args) {
        VoteShare6 v = new VoteShare6();
    ...
    public void compute_voteshare(String tag, String XMLInput) {
        try {
            ...
            out = new PrintWriter(Constants.Voteshare_Directory + tag + "/" + tag + "_vote_share_v6.csv");
            HashMap<Integer, Integer> question_sum_scores = new HashMap<>();
            for (Integer qid : question_list) {
                Integer sum_score = 0;
                for (Integer aid : question_answers.get(qid)) {
                    // ignore zero or negative votes and just get summation of positive votes
                    if (answer_score.get(aid) > 0.0)
            ...
            for (Integer aid : answer_list) {
                ...
                if (answer_score.get(aid) > 0.0 && question_sum_scores.get(answer_parent.get(aid)) != 0) {
