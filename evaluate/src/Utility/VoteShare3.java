...
public class VoteShare3 {
    public void compute_voteshare(String tag, String XMLInput) {
        try {
            ...
            out = new PrintWriter(Constants.Voteshare_Directory + tag + "/" + tag + "_vote_share_v3.csv");
            HashMap<Integer, Integer> question_min_scores = new HashMap<>();
            for (Integer qid : question_list) {
                ...
                question_min_scores.put(qid, min_score);
            }
            HashMap<Integer, Integer> question_sum_scores = new HashMap<>();
            for (Integer qid : question_list) {
                Integer sum_score = 0;
                for (Integer aid : question_answers.get(qid)) {
                    sum_score += (answer_score.get(aid) - question_min_scores.get(qid));
                }
            ...
            for (Integer aid : answer_list) {
                ...
                if (question_sum_scores.get(answer_parent.get(aid)) != 0) {
                    voteshare = ((answer_score.get(aid) - question_min_scores.get(answer_parent.get(aid))) * 1.0 /
