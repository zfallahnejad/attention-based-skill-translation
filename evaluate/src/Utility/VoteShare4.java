...
public class VoteShare4 {
    public void compute_voteshare(String tag, String XMLInput) {
        try {
            ...
            out = new PrintWriter(Constants.Voteshare_Directory + tag + "/" + tag + "_vote_share_v4.csv");
            HashMap<Integer, Integer> question_shift_scores = new HashMap<>();
            for (Integer qid : question_list) {
                Integer min_score = 1000000000;
                for (Integer aid : question_answers.get(qid)) {
                    if (answer_score.get(aid) < min_score)
                        min_score = answer_score.get(aid);
                }
                if (min_score >= 0)
                    question_shift_scores.put(qid, 0);
                else
                    question_shift_scores.put(qid, -1 * min_score);
            }
            HashMap<Integer, Integer> question_sum_scores = new HashMap<>();
            for (Integer qid : question_list) {
                Integer sum_score = 0;
                for (Integer aid : question_answers.get(qid)) {
                    sum_score += (answer_score.get(aid) + question_shift_scores.get(qid));
                }
            ...
            for (Integer aid : answer_list) {
                ...
                if (question_sum_scores.get(answer_parent.get(aid)) != 0) {
                    voteshare = ((answer_score.get(aid) + question_shift_scores.get(answer_parent.get(aid))) * 1.0 /
                            question_sum_scores.get(answer_parent.get(aid)));
                }
