...
//        searcher.setSimilarity(sim);
    ...
    /// test voteshare balog1
    public EvalResult balog1_Voteshare(String bodyTerm, boolean printDebug, String goldenName, Integer N, Double Beta, Double Lambda, HashMap<Integer, Double> voteShare, PrintWriter writer_out) throws IOException, ParseException
    {
        String goldenFile;
        if(goldenName == null)
            goldenFile = Utility.getGoldenFileName(bodyTerm);
...
//        searcher.setSimilarity(sim);
        ...
        for (Map.Entry<Integer, ArrayList<Integer>> entry : users.entrySet())
        {
            ...
            userScores.put(entry.getKey(), score);
            writer_out.println("tag:" + bodyTerm + " user:" + entry.getKey() + " score:" + score);
        }
        ...
        for (Map.Entry<Integer, Double> entry : sorted_map.entrySet())
        {
            ...
            if(printDebug)
                System.out.println("{" + entry.getKey() + " } -> "+entry.getValue());
            writer_out.println("{" + entry.getKey() + " } -> "+entry.getValue());
            lst.add(entry.getKey());
        }
        Evaluator ev = new Evaluator();
        double map = ev.map(lst, getGoldenList(goldenFile));
    ...
    public ArrayList<EvalResult> balog1ForAllTagsVoteshare(String topTag) throws IOException, ParseException
    {
        ...
        while ((line = reader.readLine()) != null)
        {
            ...
            voteShare.put(Integer.parseInt(parts[0]), Double.parseDouble(parts[1]));
        }
        reader.close();
        ...
        {
            System.out.print(tag+": ");
            EvalResult er = balog1_Voteshare(tag,false,null,10000,null,0.5, voteShare, writer_out);
            System.out.println(er.getMap()+","+er.getP1()+","+er.getP5()+","+er.getP10()+","+er.getTopUser());
            res.add(er);
        }
        writer_out.close();
    ...
    /// test voteshare balog2
    public HashMap<Integer, Double> calculateBalog2_Voteshare(Integer N, String bodyTerm, boolean printDebug, Double Beta, Double Lambda, boolean taggedAnswerOnly, HashMap<Integer, ArrayList<String>> tags, String originalTag, HashMap<Integer, Double> voteShare, PrintWriter writer_out) throws ParseException, IOException
    {
        HashMap<Integer, Double> userScores;
        if (N == null)
        {
...
//        searcher.setSimilarity(sim);
    ...
    public EvalResult balog2_Voteshare(String bodyTerm, boolean printDebug, String goldenName, Integer N, Double Beta, Double Lambda, HashMap<Integer, Double> voteShare, PrintWriter writer_out) throws IOException, ParseException
    {
        String goldenFile;
        if(goldenName == null)
            goldenFile = Utility.getGoldenFileName(bodyTerm);
        ...
        HashMap<Integer, Double > userScores = calculateBalog2_Voteshare(N, bodyTerm, printDebug, Beta, Lambda, false, null , "", voteShare, writer_out);
        ...
        for (Map.Entry<Integer, Double> entry : sorted_map.entrySet())
        {
            ...
            if(printDebug)
                System.out.println("{" + entry.getKey() + " } -> "+entry.getValue());
            writer_out.println("tag: " + bodyTerm + " { " + entry.getKey() + " } -> "+entry.getValue());
            lst.add(entry.getKey());
        }
        Evaluator ev = new Evaluator();
        double map = ev.map(lst, getGoldenList(goldenFile));
    ...
    public ArrayList<EvalResult> balog2ForAllTagsVoteshare(String topTag) throws IOException, ParseException
    {
        ...
        {
            System.out.print(tag+":");
            EvalResult er = balog2_Voteshare(tag,false,null,10000,null,0.5, voteShare, writer_out);
            System.out.println(er.getMap()+","+er.getP1()+","+er.getP5()+","+er.getP10()+","+er.getTopUser());
            res.add(er);
        }
        writer_out.close();
